...
async function getPromptQuestions(projectName, template) {
  return [
    ...
    },
    {
      ...
      when: !template,
      message:
        'Would you like to use a template? (Templates are Siapi configurations designed for a specific use case)',
    },
  ];
}
...
async function getTemplateData() {
  const response = await fetch(
    `https://api.github.com/repos/siapi/community-content/contents/templates/templates.yml`
  );
  if (!response.ok) {
    return null;
  }
